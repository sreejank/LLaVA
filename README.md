...
[📢 [LLaVA-NeXT Blog](https://llava-vl.github.io/blog/2024-01-30-llava-next/)] [[Project Page](https://llava-vl.github.io/)] [[Demo](https://llava.hliu.cc/)]  [[Data](https://github.com/haotian-liu/LLaVA/blob/main/docs/Data.md)] [[Model Zoo](https://github.com/haotian-liu/LLaVA/blob/main/docs/MODEL_ZOO.md)]f
...
## Instructions for Running on Della

The file `make_custom_json.ipynb` will make a json data file and an images folder for the dataset. You can use this to tune the prompt as well. It currently pulls grids from the human data. 

Make sure to log in to: `USER@della-vis2.princeton.edu` to start running notebooks and scripts. You'll first have to run on the vis2 node to download the pretrained models, let it crash due to insufficient memory, and then again on a compute node without internet which has more memory. 

The script for training can be run with: `source scripts/v1_5/finetune_task_lora.sh`. Modify first to change the `HF_HOME` and `HF_HUB_CACHE` directories so that your home directory doesnt run out of disk space. 

Once you run the script on `vis2` and eventually get to a memory error in pytorch, run: `salloc --nodes=1 --ntasks=1 --mem-per-cpu=30G --time=00:50:00 --gres=gpu:1` to request an interactive job on a compute node, then run the script again. You can also run it as a slurm job. 

The model should train and save in a `checkpoints` folder. To test the model, I used the CLI interface, where you give it an image within the arguments and then you can interact with the change in a command line chat. To do that, use `python -m llava.serve.cli`. Specify the base model name (I initially used `llava-v1.5-13b` but it might actually be `lmsys/vicuna-13b-v1.5`), the checkpoint to the lora-finetuned weights that the above script creates, and the image file (you can just make a new grid or choose one that was in the training data). You dont have to run it with 4bit, it didnt work for me for some reason. Do this on a compute node so that you'll have the ram. 
